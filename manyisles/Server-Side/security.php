<?php
if(!isset($_COOKIE["loggedIn"])){echo "<script>window.location.replace('/account/home?error=notSignedIn');</script>"; exit();}
if (!isset($conn)){require_once($_SERVER['DOCUMENT_ROOT']."/Server-Side/db_accounts.php");}

$uid = $_COOKIE["loggedIn"];
if (preg_match("/^[0-9]+$/", $uid)!=1) {setcookie("loggedIn", "", time() -3600, "/");header("Location: /account/home?error=notSignedIn");exit();}
