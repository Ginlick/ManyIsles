...
function checkRegger($regex, $toCheck) {
    ...
    else if (preg_match($regArray[$regex], $toCheck)==1) {
        return true;
    }
    else {
        return false;
...
function jsReg($regex) {
    global $regArray;
    //$toReturn = str_replace("\\", "\\\\", $regArray[$regex]);
    $toReturn = substr($regArray[$regex], 1);
    $toReturn = substr_replace($toReturn, "", -1);
    return($toReturn);
}

function normalizeChars($toCheck) {
    $replace = array(
        'ъ'=>'-', 'Ь'=>'-', 'Ъ'=>'-', 'ь'=>'-',
